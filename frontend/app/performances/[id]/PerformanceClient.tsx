...
export default function PerformanceClient({
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {isAdmin && (
        <section>
          <SectionHeader label={`Head ของงานแสดง (${assignedHeads.length})`}>
            <Button size="sm" variant="secondary" onClick={openAddHead}>
              + เพิ่ม Head
            </Button>
          </SectionHeader>

          {showAddHead && (
            <div className="mb-3 p-3 border border-hairline-soft rounded-[var(--radius-md)] bg-surface-card flex gap-2 items-center">
              <select
                value={selectedMemberId}
                onChange={(e) => setSelectedMemberId(e.target.value)}
                className="flex-1 px-3 py-2 text-sm border border-hairline rounded-[var(--radius-md)] bg-canvas text-ink outline-none focus:border-coral focus:ring-[3px] focus:ring-coral/20"
              >
                <option value="">-- เลือกสมาชิก --</option>
                {allMembers
                  .filter((m) => !assignedHeads.some((h) => h.id === m.id))
                  .map((m) => (
                    <option key={m.id} value={m.id}>
                      {m.nickname} ({m.generation})
                    </option>
                  ))}
              </select>
              <Button
                size="sm"
                variant="coral"
                disabled={!selectedMemberId || addHeadLoading}
                onClick={addHead}
              >
                {addHeadLoading ? "..." : "เพิ่ม"}
              </Button>
              <Button
                size="sm"
                variant="secondary"
                onClick={() => { setShowAddHead(false); setSelectedMemberId(""); }}
              >
                ยกเลิก
              </Button>
            </div>
          ...
          {assignedHeads.length === 0 ? (
            <p className="text-sm text-muted-soft">ยังไม่มี Head ที่ได้รับมอบหมาย</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {assignedHeads.map((h) => (
                <div
                ...
                >
                  <span className="text-sm font-medium text-primary">{h.nickname}</span>
                  <span className="text-xs text-muted-soft">{h.generation}</span>
                  <button
                    onClick={() => removeHead(h.id)}
                    className="text-muted hover:text-error transition-colors text-base leading-none ml-1"
                    aria-label="ลบ Head"
                  >
                    ×
                  </button>
